...
Deno.serve(async (request) => {
  ...
  if (!supabaseUrl || !serviceRoleKey || !anonKey) {
    console.error(
      "verify-online-payment: required server configuration is missing",
    );
    return jsonResponse(
  ...
  const razorpayPaymentId = firstString(
  ...
  const razorpaySignature = firstString(
  ...
  if (orderLookup.error) {
    console.warn(
      "verify-online-payment: order lookup failed",
  ...
  if (order.payment_method !== "online") {
    return jsonResponse(
      { error: "This order was not placed for online payment." },
      409,
      origin,
      allowedOrigins,
    );
  ...
  if (paymentLookup.error || !paymentLookup.data) {
    console.warn(
      "verify-online-payment: no razorpay payment attempt found for order",
      orderId,
    );
    return jsonResponse(
      { error: "No payment attempt was found for this order." },
      404,
      origin,
      allowedOrigins,
    );
  ...
  if (payment.status === "paid") {
    ...
    if (current.error) {
      console.error(
        "verify-online-payment: could not re-read already-paid order",
        current.error.code,
      );
      return jsonResponse(
    ...
    return jsonResponse(
      { order: current.data, alreadyPaid: true },
      200,
      origin,
      allowedOrigins,
    );
  ...
  if (!payment.provider_order_id) {
    return jsonResponse(
      { error: "Payment has not been started for this order." },
      409,
      origin,
      allowedOrigins,
    );
  ...
  if (providerLookup.error) {
    console.warn(
      "verify-online-payment: provider lookup failed",
  ...
  if (providerResult.provider !== "razorpay") {
    console.error(
      "verify-online-payment: unsupported provider",
  ...
  if (!signatureValid) {
    console.warn(
      "verify-online-payment: checkout signature verification failed",
      orderId,
    );
    return jsonResponse(
  ...
  try {
    ...
    console.error(
      "verify-online-payment: fetching payment from razorpay failed",
      fetchError,
    );
    return jsonResponse(
      { error: "Unable to confirm payment with the payment provider." },
      502,
      origin,
      allowedOrigins,
    );
  ...
  if (
    fetchedPayment.providerOrderId &&
    fetchedPayment.providerOrderId !== payment.provider_order_id
  ) {
    console.error(
      "verify-online-payment: razorpay payment belongs to a different order",
      orderId,
    );
    return jsonResponse(
  ...
  const resultStatus = fetchedPayment.captured
    ? "paid"
    : fetchedPayment.status === "failed"
      ? "failed"
      : "authorized";
  ...
  if (recordResult.error) {
    console.error(
      "verify-online-payment: record_payment_result failed",
  ...
  if (resultStatus !== "paid") {
    return jsonResponse(
      { error: "Payment was not captured.", order: recordResult.data },
      402,
      origin,
      allowedOrigins,
    );
