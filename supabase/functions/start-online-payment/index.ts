...
Deno.serve(async (request) => {
  ...
  if (!supabaseUrl || !serviceRoleKey || !anonKey) {
    console.error("start-online-payment: required server configuration is missing");
  ...
  const paymentAttemptId = typeof payload.paymentAttemptId === "string" ? payload.paymentAttemptId : null;
  if (!orderId || !UUID_RE.test(orderId) || !paymentAttemptId || !UUID_RE.test(paymentAttemptId)) {
    return jsonResponse({ error: "orderId and paymentAttemptId must be valid UUIDs." }, 400, origin, allowedOrigins);
  }
  ...
  if (orderLookup.error) {
    console.warn("start-online-payment: order lookup failed", orderLookup.error.code);
  ...
  if (order.payment_method !== "online" || order.status !== "payment_pending") {
    return jsonResponse({ error: "Order is not awaiting an online payment." }, 409, origin, allowedOrigins);
  }

  const amount = Number(order.grand_total);
  const currency = typeof order.currency === "string" ? order.currency : "";
  if (!Number.isFinite(amount) || amount <= 0 || !currency) {
    console.error("start-online-payment: order snapshot has an invalid amount/currency", orderId);
    return jsonResponse({ error: "Order amount could not be resolved." }, 500, origin, allowedOrigins);
  }
  ...
  if (providerLookup.error) {
    console.warn("start-online-payment: provider lookup failed", providerLookup.error.code);
    return jsonResponse(
  ...
  const providerName = typeof providerResult.provider === "string" ? providerResult.provider : "";
  ...
  if (credentials.authMode !== "api_key") {
    console.error("start-online-payment: unsupported auth mode", credentials.authMode);
    return jsonResponse({ error: "Payment provider authentication mode is not supported yet." }, 500, origin, allowedOrigins);
  }
  ...
  const attemptResult = await admin.schema("ordering").rpc("create_payment_attempt", {
  ...
  if (attemptResult.error) {
    console.warn("start-online-payment: create_payment_attempt failed", attemptResult.error.code);
    return jsonResponse(
  ...
  let providerOrderId = typeof attempt.provider_order_id === "string" ? attempt.provider_order_id : null;
  ...
  if (!providerOrderId) {
    ...
    try {
      ...
      console.error("start-online-payment: razorpay order creation failed", razorpayError);
      return jsonResponse({ error: "Unable to start payment with the payment provider." }, 502, origin, allowedOrigins);
    }
    providerOrderId = created.providerOrderId;

    const markResult = await admin.schema("ordering").rpc("mark_payment_pending", {
      p_payment_id: paymentAttemptId,
      p_provider_order_id: providerOrderId,
      p_gateway_payload: created.raw,
    });
    if (markResult.error) {
      console.error("start-online-payment: mark_payment_pending failed", markResult.error.code);
      return jsonResponse(
