-- MSG91 customer authentication uses a synthetic email only to create an
-- email-magic-link Supabase Auth session. It is not a customer contact
-- address and must never be copied into core.customers.
...
-- These addresses were generated by customer-auth-msg91 and cannot receive
-- customer communication. Clearing them preserves genuine user-entered email.
update core.customers
set email = null,
    email_verified_at = null,
    updated_at = now()
where email ~* '^msg91_[0-9]+@auth[.]invalid$';
