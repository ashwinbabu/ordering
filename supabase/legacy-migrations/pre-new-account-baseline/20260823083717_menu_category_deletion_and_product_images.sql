-- Empty categories are removed only as part of the existing optimistic menu
-- save. The wrapper keeps the established five-argument RPC contract while
-- making the deletion explicit and transactionally tied to the baseline.
...
-- Public read access is deliberate: product images are displayed in the
-- anonymous storefront. Writes remain authenticated and tenant-scoped.
...
drop policy if exists "catalog managers can upload product images" on storage.objects;
...
drop policy if exists "catalog managers can delete product images" on storage.objects;
