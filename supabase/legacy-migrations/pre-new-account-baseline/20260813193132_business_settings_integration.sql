-- Business Settings integration: secure owner/admin editing for business and
-- outlet configuration, with a single transactional save boundary.

create or replace function private.can_manage_sensitive_business_configuration(p_business_id uuid)
returns boolean
language sql
stable
security definer
...
drop trigger if exists business_locations_10_enforce_identity on core.business_locations;
...
grant update on table core.businesses, core.business_locations to authenticated;

drop policy if exists businesses_update_owners_admins on core.businesses;
create policy businesses_update_owners_admins
on core.businesses
for update
to authenticated
using ((select private.can_manage_sensitive_business_configuration(id)))
with check ((select private.can_manage_sensitive_business_configuration(id)));

drop policy if exists business_locations_update_owners_admins on core.business_locations;
create policy business_locations_update_owners_admins
on core.business_locations
for update
to authenticated
using ((select private.can_manage_sensitive_location_configuration(id)))
with check ((select private.can_manage_sensitive_location_configuration(id)));

grant delete on table ordering.delivery_zones to authenticated;

drop policy if exists delivery_zones_delete_owners_admins on ordering.delivery_zones;
create policy delivery_zones_delete_owners_admins
on ordering.delivery_zones
for delete
to authenticated
using ((select private.can_manage_sensitive_location_configuration(location_id)));
...
revoke execute on function ordering.save_business_settings(uuid, uuid, jsonb, jsonb) from public, anon;
grant execute on function ordering.save_business_settings(uuid, uuid, jsonb, jsonb) to authenticated;
