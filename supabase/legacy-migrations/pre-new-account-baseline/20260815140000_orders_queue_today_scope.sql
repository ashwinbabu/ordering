-- The Admin order queue was called with no date/status filter at all, so it
-- always returned the 50 most recent orders across the outlet's entire
-- history. The Orders screen then derived "New", "Open orders", "Completed
-- today" and "Today's sales" from that arbitrary window client-side --
-- correct only by coincidence, and silently wrong once an outlet passes 50
-- lifetime orders (a busy day truncates the window; a quiet day pulls in
-- orders from days or weeks earlier and mislabels them "today").
--
-- This adds an optional p_from/p_to window, defaulting to "today" in the
-- business's own timezone when the caller omits both. The window applies
-- only to closed orders (delivered/cancelled) -- an order that is still
-- open (placed, accepted, out for delivery, ...) must never disappear from
-- the queue just because it crossed midnight before being fulfilled.
...
begin
  ...
  if v_from is null then
    select business.timezone into v_business_timezone
    from core.businesses as business
    where business.id = p_business_id;

    v_from := date_trunc('day', now() at time zone v_business_timezone) at time zone v_business_timezone;
  ...
  return coalesce((
    ...
    from (
      ...
      where placed_order.business_id = p_business_id
        ...
        and (
          placed_order.status not in ('delivered', 'cancelled')
          or (placed_order.created_at >= v_from and placed_order.created_at < v_to)
        )
        and (
