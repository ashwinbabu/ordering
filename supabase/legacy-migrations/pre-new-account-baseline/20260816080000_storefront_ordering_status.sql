-- Closes the ordering-status loop between Admin and the Storefront.
--
-- Two halves:
--   1. get_storefront_menu now reports whether the outlet is accepting orders,
--      so a customer loading the menu starts from the correct state.
--   2. A narrow Realtime broadcast so an operator toggling the kill switch is
--      reflected without a reload.
--
-- ordering.restaurant_settings deliberately stays private. It holds commercial
-- data (skrowia_commission_rate, aggregator_benchmark_rate, tax_rate), so this
-- migration adds no anon grant, no RLS policy and no publication membership.
-- The security-definer function reads the flag on the customer's behalf, and
-- the trigger broadcasts only the single boolean.
...
comment on function ordering.get_storefront_menu(uuid) is
  'Returns a location-scoped customer menu and whether the outlet is accepting orders. Invalid or inactive locations return null.';
...
-- Realtime half.
--
-- realtime.send() is used rather than realtime.broadcast_changes() because the
-- latter ships the whole record, which would put commission and tax rates on a
-- customer-facing channel. The payload here is only what the customer already
-- sees on the menu.
--
-- The private flag is false, so this is a public channel: no realtime.messages
-- RLS policy is required, and the anonymous storefront can subscribe with the
-- publishable key. The client must subscribe WITHOUT `private: true` or the
-- two ends will not match.
...
comment on function ordering.broadcast_ordering_status() is
  'Broadcasts only the accepting-orders flag for a location. Does not expose any other restaurant_settings column.';

drop trigger if exists restaurant_settings_ordering_status_broadcast
  on ordering.restaurant_settings;

-- The when clause keeps unrelated Business Settings saves (tax, commercials,
