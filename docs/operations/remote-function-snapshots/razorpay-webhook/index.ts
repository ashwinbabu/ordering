...
function boundedPayload(paymentEntity: Record<string, unknown>): Record<string, unknown> {
  const {
    id, order_id, status, method, amount, currency, fee, tax, captured, email, contact, created_at,
  } = paymentEntity;
  return { id, order_id, status, method, amount, currency, fee, tax, captured, email, contact, created_at };
}
...
Deno.serve(async (request) => {
  ...
  const webhookConfigLookup = await admin
    .schema("ordering")
    .rpc("get_payment_provider_webhook_config", { p_provider_configuration_id: providerConfigurationId });
  if (webhookConfigLookup.error) {
    console.warn("razorpay-webhook: webhook config lookup failed", webhookConfigLookup.error.code);
    return json({ error: "Unknown payment provider configuration." }, statusForPgErrorCode(webhookConfigLookup.error.code));
  }

  const webhookContext = webhookContextFromResolution(webhookConfigLookup.data as Record<string, unknown>);
  if (webhookContext.provider !== "razorpay") {
    console.error("razorpay-webhook: provider configuration is not razorpay", webhookContext.provider);
  ...
  if (!signatureValid) {
    console.warn("razorpay-webhook: invalid signature for provider configuration", providerConfigurationId);
  ...
  const handledEvents = new Set(["payment.captured", "payment.failed", "order.paid"]);
  ...
  if (!paymentEntity || typeof paymentEntity.id !== "string" || typeof paymentEntity.order_id !== "string") {
    console.warn("razorpay-webhook: event carried no usable payment entity", eventType);
    return json({ ok: true, ignored: "no_payment_entity" }, 200);
  }

  if (!providerEventId) {
    console.warn("razorpay-webhook: event carried no event id, refusing to process without one");
  ...
  if (paymentLookup.error) {
    console.error("razorpay-webhook: payment lookup failed", paymentLookup.error.message);
  ...
  if (!paymentLookup.data) {
    ...
    console.warn("razorpay-webhook: no matching payment for provider_order_id", paymentEntity.order_id);
    return json({ ok: true, ignored: "unmatched_order" }, 200);
  }

  const resultStatus = eventType === "payment.failed"
    ? "failed"
    : paymentEntity.status === "captured"
    ? "paid"
    : "authorized";

  const occurredAtEpoch = typeof event.created_at === "number" ? event.created_at : null;
  ...
  const recordResult = await admin.schema("ordering").rpc("record_payment_result", {
    ...
    p_gateway_fee: typeof paymentEntity.fee === "number" ? paymentEntity.fee / 100 : null,
    p_gateway_tax: typeof paymentEntity.tax === "number" ? paymentEntity.tax / 100 : null,
    p_occurred_at: occurredAtEpoch ? new Date(occurredAtEpoch * 1000).toISOString() : new Date().toISOString(),
  });

  if (recordResult.error) {
    console.error("razorpay-webhook: record_payment_result failed", recordResult.error.code, recordResult.error.message);
