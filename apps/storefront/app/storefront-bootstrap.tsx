...
const hostname = import.meta.env.VITE_STOREFRONT_HOSTNAME_OVERRIDE ?? window.location.hostname;
...
export function StorefrontBootstrap({ children }: { children: ReactNode }) {
  ...
  if (contextResource.status === "pending") {
    return <main className="ordering-app"><section className="customer-empty-state" aria-busy="true"><h1>Loading</h1><p>Finding your restaurant.</p></section></main>;
  }

  if (contextResource.status === "error") {
    return <main className="ordering-app"><section className="customer-empty-state" role="alert"><h1>Something went wrong</h1><p>{contextResource.error.message}</p><button className="primary-button" type="button" onClick={() => void contextResource.refetch()}>Try again</button></section></main>;
  }

  if (!contextResource.data) {
    return <main className="ordering-app"><section className="customer-empty-state" role="alert"><h1>Restaurant not found</h1><p>This link doesn&rsquo;t match an active restaurant.</p></section></main>;
  }
