...
export function MenuRoute() {
  const { cartItemCount, cartQuantities, cartTotal, menu, openAuth, openProductConfiguration, adjustConfigurableProductQuantity, changeSimpleProductQuantity, venue, viewProduct } = useOutletContext<StorefrontLayoutContext>();
  ...
  return <div className="ordering-app">
    <VenueHeader cartItemCount={cartItemCount} venue={venue} onGoToCart={openCart} onOpenAccount={requestAccountAuthentication} />
    <MenuScreen cartItemCount={cartItemCount} cartQuantities={cartQuantities} cartTotal={cartTotal} footer={<VenueFooter venue={venue} />} isAcceptingOrders={venue.isAcceptingOrders} locationName={venue.locationName} menu={menu} onAddProduct={openProductConfiguration} onAdjustQuantity={adjustConfigurableProductQuantity} onGoToCart={openCart} onQuantityChange={changeSimpleProductQuantity} onViewProduct={(product) => viewProduct(product.id)} orderingStatus={venue.orderingStatus} />
  </div>;
}
