...
export function AddressesRoute() {
  const { customerAddressesResource, removeAddress, saveAddress, savedAddresses, venue } = useOutletContext<StorefrontLayoutContext>();
  ...
  return <SavedAddressesScreen
    ...
    onSave={saveAddress}
    state={customerAddressesResource.isPending ? "loading" : customerAddressesResource.isError ? "error" : "ready"}
    venue={venue}
  />;
}
