  ...
  return (
    <div className="sheet-layer" role="presentation">
      ...
      <section
        ...
        aria-modal="true"
        className="bottom-sheet product-detail-sheet"
        role="dialog"
      >
        {product.imageUrl ? (
          ...
          <button
            aria-label="Close details"
            className="icon-button sheet-close sheet-close--floating"
            type="button"
            onClick={onClose}
          >
            <X aria-hidden="true" size={20} />
        ...
        <div className="detail-content">
          ...
          {productTag ? (
            <span className="eyebrow-tag detail-badge">{productTag}</span>
          ) : null}
          <p className="detail-description">{product.description}</p>
          <div className="detail-facts">
            <span>
              <Clock aria-hidden="true" size={17} strokeWidth={1.8} /> Made to
              order
            </span>
            <span>
              <Store aria-hidden="true" size={17} strokeWidth={1.8} /> From{" "}
              {shortLocationName}
            </span>
          </div>
        </div>
        <div className="sheet-cta">
          <button
            ...
            disabled={!isAvailable || !isAcceptingOrders}
            type="button"
            onClick={addAndClose}
          >
            {isAvailable
              ? `${isConfigurable ? "Choose options" : "Add to Cart"} · ${formatRupees(product.price)}`
