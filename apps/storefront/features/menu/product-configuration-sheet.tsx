...
import {
  ...
  type MenuProduct,
  type StorefrontMenuOptionGroup,
...
function selectionsFor(
  groups: StorefrontMenuOptionGroup[],
  selections: CartLineOptionSelection[],
): SelectionByGroup {
...
function isGroupValid(
  group: StorefrontMenuOptionGroup,
...
export function ProductConfigurationSheet({
  initialSelections = [],
  isAcceptingOrders,
  onClose,
  onConfirm,
  product,
}: ProductConfigurationSheetProps) {
  const optionGroups = product.optionGroups ?? [];
  const [selectedOptionIds, setSelectedOptionIds] = useState<SelectionByGroup>(
    () => selectionsFor(optionGroups, initialSelections),
  );
  const isValid = optionGroups.every((group) =>
    isGroupValid(group, selectedOptionIds[group.id]),
  );
  ...
  function toggleOption(group: StorefrontMenuOptionGroup, optionId: string) {
    setSelectedOptionIds((current) => {
      ...
      if (group.selectionType === "single") {
        return {
          ...current,
          [group.id]: currentIds.includes(optionId) ? [] : [optionId],
        };
      }
  ...
  return (
    <div className="sheet-layer" role="presentation">
      <button
        aria-label={`Close ${product.name} configuration`}
      ...
      <section
        ...
        aria-modal="true"
        className="bottom-sheet configuration-sheet"
        role="dialog"
      >
              ...
              <fieldset className="option-group" key={group.id}>
                <legend>
                  <span>
                    ...
                    <small>{requirementText(group)}</small>
                  </span>
                ...
                <div className="option-choices">
                  {group.options.map((option) => {
                    ...
                    const atMaximum =
                      ...
                      selection.length >= group.maxSelections;
                    return (
                      <button
                      ...
                      >
                        <span
                          className={`choice-control choice-control--${group.selectionType}`}
                          aria-hidden="true"
                        >
                          {checked ? (
        ...
        <div className="sheet-cta configuration-cta">
          ...
          <button
            ...
            disabled={!isValid || !isAcceptingOrders}
            type="button"
            onClick={() => {
