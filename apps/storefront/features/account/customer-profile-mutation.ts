...
import { readNullableString, readRecord, readString } from "../../lib/supabase/json-parsing";
 ...
 */
export async function updateCustomerProfile(displayName: string, email: string | null): Promise<UpdatedCustomerProfile> {
  const result = await callUntypedRpc(getSupabaseClient().schema("core"), "update_customer_profile", {
    p_display_name: displayName,
    p_email: email,
  });
  if (result.error) throw result.error;

  const profile = readRecord(result.data as never, "The updated customer profile");
  return {
    id: readString(profile.id, "The customer ID"),
    displayName: readNullableString(profile.display_name, "The customer display name") ?? "",
