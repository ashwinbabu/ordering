...
const busyCopy: Partial<
  Record<CheckoutPhase, { title: string; body: string }>
> = {
  quoting: {
    title: "Checking the latest price",
    body: "Confirming delivery fees and availability.",
  },
  creating_order: {
    title: "Placing your order",
    body: "Setting things up with the kitchen.",
  },
  preparing_payment: {
    title: "Preparing payment",
    body: "We're getting your payment ready.",
  },
  confirming: {
  ...
  placing_order: {
    title: "Placing order",
    body: "Letting the kitchen know your order is on its way.",
  },
};
...
export function PaymentFlowScreen(props: PaymentFlowScreenProps) {
  ...
  if (phase === "quote_changed")
    return (
      <PaymentState
        icon={<CircleDollarSign />}
        title="Your total has changed"
        body={`The final total is ${formatRupees(props.updatedAmount ?? 0)}. This can happen when delivery fees, availability or offers change.`}
        venue={venue}
        actions={
          <>
            <button
            ...
            <button
              ...
              type="button"
              onClick={props.onBackToRestaurant}
            >
              Review cart
            </button>
          </>
  ...
  if (busyCopy[phase])
    return (
      <PaymentPage venue={venue}>
        <Securing title={busyCopy[phase]!.title} body={busyCopy[phase]!.body} />
      </PaymentPage>
    );
  if (phase === "awaiting_provider")
    return (
      <PaymentPage venue={venue}>
        <GatewaySheet order={order} />
      </PaymentPage>
    );
  if (phase === "confirmed") {
    ...
    return (
      <PaymentPage venue={venue}>
        <Securing
          ...
          title={isCashOnDelivery ? "Order confirmed" : "Payment confirmed"}
          body="Your order is being opened."
        />
      </PaymentPage>
    );
  }
  if (phase === "pending")
    return (
      <PaymentState
        icon={<Clock3 />}
        title="We&rsquo;re confirming your payment"
        body="This is taking a little longer than usual. Don&rsquo;t make another payment yet."
        venue={venue}
        actions={
          <div className="review-status-control">
            <span>Still checking</span>
            <p>
              We&rsquo;ll keep checking this payment automatically. You can also
              check right now.
            </p>
            <button
              className="secondary-button"
  ...
  if (phase === "failed")
    return (
      <PaymentState
        ...
        body="Your payment was not successful."
        venue={venue}
        actions={
          <button
            className="primary-button"
  ...
  if (phase === "cancelled")
    return (
      <PaymentState
        ...
        body="Your order hasn't been paid yet."
        venue={venue}
        actions={
          <>
            <button
  ...
  if (phase === "verification_error")
    return (
      <PaymentState
        ...
        body="We couldn't check your payment status right now. If you already completed the payment, don't pay again yet."
        venue={venue}
        actions={
          <>
            <button
  ...
  return (
    <PaymentState
      ...
      body={
        props.startError ??
        (order
          ? "Your order details are safe. Please try again."
          : "Please try again.")
      }
      venue={venue}
      actions={
...
function PaymentState({
  ...
  return (
    <PaymentPage venue={venue}>
      <section className="payment-state" aria-live="polite">
        <span className="payment-state__icon">{icon}</span>
        <p className="section-kicker">Secure payment</p>
        <h1>{title}</h1>
        <p>{body}</p>
        {actions ? (
          <div className="payment-state__actions">{actions}</div>
        ) : null}
      </section>
    </PaymentPage>
  );
...
function Securing({
  ...
  return (
    <section className="securing-page" aria-live="polite" aria-busy={!done}>
      <div className="securing-animation">
        {!done ? <span className="securing-ring" aria-hidden="true" /> : null}
        <span className="securing-check">
...
function GatewaySheet({ order }: { order: PaymentPendingOrder | null }) {
  return (
    <section className="gateway-sheet">
      ...
      <div className="gateway-copy">
        ...
        <p>
          Complete the payment in the window that opened. When you return,
          we&rsquo;ll check the status before confirming your order.
        </p>
        {order ? (
          <div>
            <span>Amount</span>
      ...
      <div className="review-notice">
        ...
        <span>
          Only pay once. If the payment window doesn&rsquo;t open, come back and
          try again — we won&rsquo;t charge you twice.
        </span>
      </div>
    </section>
  );
