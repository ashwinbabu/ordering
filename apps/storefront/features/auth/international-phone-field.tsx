...
const popularCountries = popularCountryIso2
...
export function InternationalPhoneField({
  ...
  const popularIso2 = new Set(popularCountryIso2);
  const visibleCountries = showAllCountries
    ? supportedCountries
    : selectedCountry && !popularIso2.has(selectedCountry.iso2)
      ? [selectedCountry, ...popularCountries]
      : popularCountries;
  ...
  useLayoutEffect(() => {
    ...
    setMenuPosition({
      left: Math.min(trigger.left, window.innerWidth - 253),
      top: openBelow
        ? trigger.bottom + 7
      ...
      >
        <div className="phone-input__country">
          <button
            ...
            aria-label={`Country: ${selectedCountry?.name ?? "Select country"}`}
        ...
        <input
          ...
          inputMode="tel"
          onChange={(event) =>
            onChange(normalizePhoneInput(event.target.value, value.countryIso2))
          }
