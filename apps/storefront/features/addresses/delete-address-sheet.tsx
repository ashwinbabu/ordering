...
export function DeleteAddressSheet({
  address,
  error,
  onCancel,
  onConfirm,
}: DeleteAddressSheetProps) {
  const label = address.customLabel || address.label;
  return (
    <div className="sheet-layer" role="presentation">
      <button
        aria-label="Close remove address confirmation"
      ...
      >
        <div className="sheet-title-row">
          <div>
            ...
            <h2 id="remove-address-title">Remove this address?</h2>
          </div>
          <button
        ...
        <div className="confirmation-sheet__body">
          <strong>{label}</strong>
          <p>
            {address.line1}, {address.locality}, {address.city}
          </p>
          <small>You can add it again later if needed.</small>
        </div>
        {error ? (
