...
export function AddressCard({ address, onDelete, onEdit }: AddressCardProps) {
  ...
  return <article className="saved-address-card">
    <div className="saved-address-card__icon"><MapPin aria-hidden="true" size={20} /></div>
    <div className="saved-address-card__copy">
      <div><h2>{label}</h2>{address.isDefault ? <span>Default</span> : null}</div>
    ...
    <div className="saved-address-card__actions">
      ...
      <button className="danger-text-button" type="button" onClick={onDelete}><Trash2 aria-hidden="true" size={16} />Delete</button>
    </div>
  </article>;
}
