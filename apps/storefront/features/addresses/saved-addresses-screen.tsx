...
export function SavedAddressesScreen({
  ...
  async function saveAddress(draft: AddressDraft) {
    try {
      ...
      setFormError(
        errorMessage(error, "Couldn't save this address. Please try again."),
      );
    }
  }

  return (
    <main className="ordering-app customer-page saved-addresses-page">
      <CustomerPageHeader
      ...
      <div className="customer-page-shell">
        {state === "loading" ? (
          <section className="customer-empty-state">
            <h2>Loading saved addresses</h2>
            <p>Just a moment while we get your delivery details.</p>
          </section>
        ) : null}
        {state === "error" ? (
          <section className="customer-empty-state">
            <h2>Couldn’t load addresses</h2>
            <p>Please try again. Your saved addresses are unchanged.</p>
        ...
        {state === "ready" && addresses.length === 0 ? (
          <section className="customer-empty-state">
            ...
            <p>Save an address to make checkout faster next time.</p>
            <button
        ...
        {state === "ready" && addresses.length > 0 ? (
          <section
            className="saved-addresses-list"
          ...
          >
            ...
            <button
              className="add-address-row saved-addresses-list__add"
              type="button"
              onClick={openCreate}
            >
              <Plus aria-hidden="true" size={19} />
      ...
      {deletingAddress ? (
        <DeleteAddressSheet
          ...
          error={deleteError}
          onCancel={() => setDeletingAddress(undefined)}
          onConfirm={() => {
            void Promise.resolve(onDelete(deletingAddress.id))
              ...
              .catch((error: unknown) =>
                setDeleteError(
                  errorMessage(
                    error,
                    "Couldn't delete this address. Please try again.",
                  ),
                ),
              );
          }}
