...
export function AddressSelectorSheet({ addresses, onAdd, onClose, onSelect, selectedAddressId }: AddressSelectorSheetProps) {
  return <div className="sheet-layer" role="presentation" onMouseDown={onClose}>
    ...
    <section className="bottom-sheet address-selector-sheet" role="dialog" aria-modal="true" aria-labelledby="address-selector-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="address-selector">
        <div className="address-selector__list" role="radiogroup" aria-label="Saved addresses">
          {addresses.map((address) => {
            const isSelected = selectedAddressId === address.id;
            return <button aria-checked={isSelected} className={isSelected ? "is-active" : undefined} key={address.id} role="radio" type="button" onClick={() => onSelect(address)}>
              <span className="selector-check" aria-hidden="true">{isSelected ? <Check aria-hidden="true" size={13} strokeWidth={2.6} /> : null}</span>
              <span>
                <strong>{address.customLabel || address.label}</strong>
                {address.isDefault ? <small>Default</small> : null}
                <em>{address.line1}</em>
                <em>{address.locality}, {address.city}</em>
              </span>
            </button>;
          })}
        </div>
        <button className="add-another-address" type="button" onClick={onAdd}><Plus aria-hidden="true" size={17} />Add another address</button>
      </div>
    </section>
  </div>;
}
