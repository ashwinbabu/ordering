...
function useAddressMutation<TArgs, TResult>(
  ...
  return useMutation({
    mutationFn: async (args: TArgs) => {
      if (!customerId)
        throw new Error("Sign in to manage your saved addresses.");
...
export function useSaveCustomerAddressMutation(
  customerId: string | null,
  context: StorefrontContext = storefrontContext,
) {
  return useAddressMutation<
    { draft: AddressDraft; addressId?: string },
    string
  >(customerId, context, async ({ draft, addressId }, customerBusinessId) => {
    if (addressId) {
