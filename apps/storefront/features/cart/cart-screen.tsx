...
import {
  checkoutEligibilityMessage,
  evaluateCheckoutEligibility,
} from "./cart-eligibility";
import {
  cartErrorMessage,
  useSetCartCouponMutation,
} from "./storefront-cart-mutations";
import {
  ensureCurrentCustomerCartReady,
...
function computeDisplayTax(
  ...
  if (settings.taxMode === "inclusive")
    return (
      Math.round(
        ((netFood * settings.taxRate) / (100 + settings.taxRate)) * 100,
      ) / 100
    );
...
export function CartScreen({
  ...
  const [addressSheet, setAddressSheet] = useState<"selector" | "form" | null>(
    null,
  );
  ...
  const previousLineTotals = useRef<
    Map<string, { quantity: number; lineTotal: number }>
  >(new Map());
  ...
  useEffect(() => {
    ...
    void queryClient.invalidateQueries({
      queryKey: storefrontCartQueryKey(customerId),
      exact: true,
    });
    ...
    if (previous.size > 0) {
      const changed = lines.some((line) => {
        ...
        return (
          ...
          before.lineTotal !== line.lineTotal
        );
      });
      if (changed) setPriceChangeNotice(true);
    }
    previousLineTotals.current = new Map(
      lines.map((line) => [
        line.id,
        { quantity: line.quantity, lineTotal: line.lineTotal },
      ]),
    );
  }, [lines]);
  ...
  const selectedAddress =
    savedAddresses.find((address) => address.id === effectiveAddressId) ??
    (pendingAddress?.id === effectiveAddressId ? pendingAddress : undefined);
  const appliedCouponCode = cart?.coupon?.code;
  const couponDiscountQuery = useCartCouponDiscountQuery(
    appliedCouponCode,
    subtotal,
  );
  const discount =
    appliedCouponCode && couponDiscountQuery.data?.valid
      ? (couponDiscountQuery.data.discountAmount ?? 0)
      : 0;
  ...
  const effectiveCashOnDelivery = cashOnlyMode
    ? true
    : onlineOnlyMode
      ? false
      : isCashOnDelivery;
  ...
  const deliveryDestination =
    ...
    selectedAddress?.longitude !== undefined
      ...
      : undefined;
  const deliveryQuoteQuery = useDeliveryQuoteQuery(
    deliveryDestination,
    netFoodSubtotal,
  );
  const isDeliveryQuoteLoading =
    fulfilment === "delivery" &&
    Boolean(selectedAddress) &&
    Boolean(deliveryDestination) &&
    deliveryQuoteQuery.isPending;
  const deliveryFee =
    fulfilment === "pickup"
      ? 0
  ...
  const total =
    settings?.taxMode === "exclusive"
      ? netFoodSubtotal + taxes + (deliveryFee ?? 0)
      : netFoodSubtotal + (deliveryFee ?? 0);
  const needsAddress = fulfilment === "delivery" && !selectedAddress;
  const isDeliveryUnserviceable =
    fulfilment === "delivery" &&
    Boolean(selectedAddress) &&
    Boolean(deliveryDestination) &&
    deliveryQuoteQuery.data?.serviceable === false;
  ...
  function openAddressForm(address?: DeliveryAddress) {
    setEditingAddress(address);
    setAddressSheet("form");
  }
  ...
  function continueToPayment() {
    if (needsAddress) {
      ...
      if (savedAddresses.length) setAddressSheet("selector");
      else openAddressForm();
      document
    ...
    const checkoutRequest: CheckoutRequest = {
      ...
      items: lines.map((line) => ({
        ...
        quantity: line.quantity,
        unitPrice: line.unitPrice,
        selectedOptions: line.options.map((option) => option.name),
      })),
    ...
    async function proceedToCheckout(
      ...
      if (effectiveCashOnDelivery) onCashCheckoutAttempt(request);
      else onCheckoutAttempt(request);
    }
  ...
  const zoneExtraMinutes =
    deliveryDestination && deliveryQuoteQuery.data?.distanceKm !== undefined
      ? deliveryQuoteQuery.data.distanceKm <= 2
        ? 5
        : deliveryQuoteQuery.data.distanceKm <= 5
          ? 10
          : 15
      : 0;
  const estimatedArrivalMinutes =
    fulfilment === "pickup"
      ? kitchenPrepMinutes
      : kitchenPrepMinutes + zoneExtraMinutes;
  const showEstimatedArrival =
    !needsAddress &&
    (fulfilment === "pickup" ||
      (Boolean(deliveryDestination) &&
        deliveryQuoteQuery.data?.serviceable === true));
    ...
    return (
      <main className="cart-page">
        <section className="customer-empty-state" aria-busy="true">
          ...
          <p>Getting your latest items and pricing.</p>
        </section>
      </main>
    );
  }
  ...
  return (
    <main className="cart-page">
      ...
      <div className="cart-content">
        {!isOnline ? (
          <p className="address-error" role="alert">
            You&rsquo;re offline. You can still review and edit your cart;
            pricing and checkout need a connection.
          </p>
        ) : null}
        {cartError ? (
          <p className="address-error" role="alert">
        ...
        {priceChangeNotice ? (
          <p className="address-error" role="status">
            Some prices were just updated to current menu pricing.{" "}
            <button
              className="text-button"
              type="button"
              onClick={() => setPriceChangeNotice(false)}
            >
              Dismiss
            </button>
          </p>
        ...
        {settings && !settings.orderingEnabled ? (
          <p className="address-error" role="alert">
            {venue.locationName} has paused ordering. Your cart is saved -- you
            can check out once ordering resumes.
          </p>
        ) : null}
        {settings &&
        ...
        !settings.acceptOrdersWhenClosed ? (
          <p className="address-error" role="alert">
            {venue.locationName} is closed right now. Your cart is saved for
            when it reopens.
          </p>
        ) : null}

        <section
          className="cart-section cart-items"
          aria-labelledby="cart-order-title"
        >
          ...
          <div className="cart-item-list">
            {lines.map((line) => (
              <article
                className="cart-item"
                style={line.isAvailable ? undefined : { opacity: 0.55 }}
                key={line.id}
              >
                <div className="cart-item__body">
                  <div className="cart-item__top">
                    <div>
                      ...
                      {!line.isAvailable ? (
                        <small className="address-error" role="alert">
                          {unavailableCopy(line)}
                        </small>
                      ) : null}
                    </div>
                  ...
                  <div className="cart-item__actions">
                    <div className="cart-item__links">
                      {line.isAvailable && line.hasOptionGroups ? (
                        <button
                          type="button"
                          onClick={() => onEditConfiguration(line.id)}
                        >
        ...
        <section
          className="cart-section cart-section--supporting instructions-section"
          aria-labelledby="instructions-title"
        >
          <div className="cart-section__title cart-section__title--stacked">
            <div>
              <h2 id="instructions-title">Special instructions</h2>
              <p>For the kitchen, not a replacement for item options.</p>
            </div>
            <span>{specialInstructions.length}/200</span>
          </div>
          <label className="sr-only" htmlFor="special-instructions">
        ...
        <section
          className="cart-section cart-section--supporting coupon-section"
          aria-label="Add a discount code"
        >
          {appliedCouponCode ? (
            <div className="coupon-applied">
              ...
              <span>
                ...
                <small>
                  {couponDiscountQuery.isPending
                    ? "Checking coupon…"
                    : couponDiscountQuery.data?.valid
                      ? `Coupon applied · -${formatRupees(discount)}`
              ...
              <button
                ...
                disabled={setCartCoupon.isPending}
              >
                Remove
              </button>
            </div>
            ...
            <div className="coupon-expanded">
              ...
              <div className="coupon-input-row">
                ...
                <button
                  type="button"
                  disabled={!couponCode || setCartCoupon.isPending || !isOnline}
                  onClick={applyCoupon}
                >
                  {setCartCoupon.isPending ? "Applying…" : "Apply"}
                </button>
              </div>
              {setCartCoupon.isError ? (
                <p className="address-error" role="alert">
                  {cartErrorMessage(
                    setCartCoupon.error,
            ...
            >
              ...
              <span>
                <strong>Add a discount code</strong>
                <small>Have an offer code?</small>
              </span>
        ...
        <section
          className="cart-section cart-section--supporting fulfilment-section"
          aria-labelledby="fulfilment-title"
        >
          <div className="cart-section__title cart-section__title--stacked">
            <div>
              <p className="section-kicker">How should we get it to you?</p>
              <h2 id="fulfilment-title">Fulfilment</h2>
            </div>
          </div>
          <div
            className="fulfilment-toggle"
            role="radiogroup"
          ...
          >
            <button
              ...
              className={fulfilment === "delivery" ? "is-active" : undefined}
            ...
            <button
              type="button"
              role="radio"
              aria-checked={fulfilment === "pickup"}
              disabled={!pickupSupported}
              className={fulfilment === "pickup" ? "is-active" : undefined}
            ...
            >
              ...
              <span>
                ...
                <small>
                  {pickupSupported
                    ? `From ${venue.locationName.replace(", Goa", "")}`
                    : "Not offered here"}
                </small>
              </span>
              <span className="radio-dot" aria-hidden="true" />
          ...
          {fulfilment === "pickup" ? (
            <div className="pickup-card">
              ...
              <div>
                <strong>Collect from {venue.displayName}</strong>
                <p>{venue.address}</p>
              </div>
            </div>
          ) : null}
          {fulfilment === "delivery" ? (
            ...
            >
              <h3 id="delivery-address-title" className="sr-only">
                Delivery address
              </h3>
              {selectedAddress ? (
                ...
                ) : isDeliveryUnserviceable ? (
                  <p className="address-error" role="alert">
                    We can&rsquo;t deliver to this address yet. Choose a
                    different address or switch to pickup.
                  </p>
                ) : (
                  <SelectedAddressCard
                    address={selectedAddress}
                    deliveryFee={deliveryDestination ? deliveryFee : undefined}
                    distanceKm={
                      deliveryDestination
                        ? deliveryQuoteQuery.data?.distanceKm
                        : undefined
                    }
                    venueDisplayName={venue.displayName}
                    onChange={() => setAddressSheet("selector")}
                    onEdit={() => openAddressForm(selectedAddress)}
                  />
                )
              ) : (
                <button
                  ...
                  onClick={() =>
                    savedAddresses.length
                      ? setAddressSheet("selector")
                      : openAddressForm()
                  }
                  aria-describedby={
                    addressInvalid ? "address-error" : undefined
                  }
                >
                  <span className="address-card-icon">
                  ...
                  <span>
                    ...
                    <small>Needed to calculate fee and ETA</small>
                  </span>
                </button>
              )}
              {addressInvalid && !selectedAddress ? (
        ...
        <section
          className="cart-section summary-section"
        ...
        >
          ...
          {settings && netFoodSubtotal < settings.minimumOrderValue ? (
            <p className="address-error" role="alert">
              Add {formatRupees(settings.minimumOrderValue - netFoodSubtotal)}{" "}
              more to meet the {formatRupees(settings.minimumOrderValue)}{" "}
              minimum order.
            </p>
          ) : null}
          <p className="summary-note">
            Final prices are rechecked before the secure payment hand-off.
          </p>
          {onlineOnlyMode ? null : (
      ...
      <div className="checkout-dock">
        <div className="checkout-dock__inner">
          {showEstimatedArrival ? (
            <p className="checkout-dock__eta">
              <Clock3 aria-hidden="true" size={14} strokeWidth={1.9} />
              Estimated {fulfilment === "pickup" ? "ready" : "arrival"} in{" "}
              {estimatedArrivalMinutes}–{estimatedArrivalMinutes + 5} min
            </p>
          ) : null}
          <button
            className="primary-button"
            type="button"
            disabled={!needsAddress && !eligibility.canCheckout}
      ...
      {addressSheet === "form" ? (
        <div className="sheet-layer" role="presentation">
          ...
          />
          <section
            className="bottom-sheet address-form-sheet"
            role="dialog"
            aria-modal="true"
            aria-label={editingAddress ? "Edit address" : "Add an address"}
          >
            <AddressForm
...
function unavailableCopy(line: CartLineView) {
  if (line.unavailableReason === "product-removed")
    return "This item is no longer on the menu.";
  if (line.unavailableReason === "option-unavailable")
