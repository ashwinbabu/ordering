import type { CartLineView, ServerCart, ServerCartItem } from "../../domain/cart";
import type { CartLineOptionSelection, Menu, MenuProduct } from "../../domain/storefront";
...
function reconcileItem(item: ServerCartItem, menuProductsById: Map<string, MenuProduct>): CartLineView {
  ...
  const currentOptionIds = new Set(
    (menuProduct.optionGroups ?? []).flatMap((group) => group.options.filter((option) => option.available).map((option) => option.id)),
  );
  const hasRemovedOption = item.options.some((option) => !currentOptionIds.has(option.optionId));
...
export function reconcileCartLines(cart: ServerCart | undefined, menu: Menu | null): CartLineView[] {
  if (!cart) return [];
  const menuProductsById = new Map(menu?.products.map((product) => [product.id, product]) ?? []);
...
export function selectionsFromServerItem(item: ServerCartItem, product: MenuProduct): CartLineOptionSelection[] {
  return item.options.flatMap((option) => {
    for (const group of product.optionGroups ?? []) {
      const match = group.options.find((candidate) => candidate.id === option.optionId);
      if (match) {
        return [{ groupId: group.id, optionId: option.optionId, groupName: group.name, optionName: match.name, priceDelta: option.priceDelta }];
      }
    }
    return [];
  });
