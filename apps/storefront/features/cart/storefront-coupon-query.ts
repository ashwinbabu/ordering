...
// on the client.
export function useCartCouponDiscountQuery(code: string | undefined, eligibleFoodSubtotal: number, context: StorefrontContext = storefrontContext) {
  ...
  return useQuery({
    queryKey: ["storefront", "coupon-discount", context.businessId, context.locationId, code, roundedSubtotal],
    queryFn: () => validateCoupon({ businessId: context.businessId, locationId: context.locationId, code: code!, eligibleFoodSubtotal: roundedSubtotal }),
