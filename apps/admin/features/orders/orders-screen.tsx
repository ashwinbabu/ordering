      ...
      {open && (
        <div className="date-range-menu" role="menu">
          <div className="date-range-presets">
            ...
            <button
              type="button"
              onClick={() => applyPreset({ from: yesterday, to: today })}
            >
              Yesterday &amp; today
            </button>
            <button
              type="button"
              onClick={() => applyPreset({ from: last3, to: today })}
            >
              Last 3 days
            </button>
            <button
              type="button"
              onClick={() => applyPreset({ from: last7, to: today })}
            >
              Last 7 days
            </button>
          </div>
...
function OrderCard({
  ...
  return (
    <article
      className={`order-card ${order.status === "New" ? "new-order" : ""}`}
    >
      ...
      <section className="order-zone order-items-zone">
        ...
        <div className="bill-row">
          <span>
            Total{" "}
            <small className={order.paid ? "paid" : "cod"}>
        ...
        {action ? (
          <button
            className="primary-button order-main-action"
            onClick={onProgress}
            disabled={busy}
          >
      ...
      <section className="order-zone order-delivery-zone">
        ...
        <div className="delivery-actions">
          ...
          {cancellable && (
            <button className="destructive-link" onClick={onCancel}>
              Cancel order
            </button>
          )}
        </div>
...
export function OrdersPage({
  ...
  const salesToday = deliveredToday.reduce(
    ...
    <div className="page orders-page">
      <div className="orders-command-row">
        ...
        <div className={`ordering-control ${orderingOpen ? "open" : "paused"}`}>
          <span className="ordering-control-status">
            <Store size={16} />
            {orderingOpen ? "Accepting orders" : "Orders paused"}
          </span>
          <div className="ordering-toggle-wrap">
            ...
            <Toggle
              ...
              label="Toggle restaurant ordering"
            />
          </div>
        </div>
      </div>
        ...
        <div className="status-tabs" role="tablist" aria-label="Order status">
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                {tab}
                <span aria-label={`${count} orders`}>{count}</span>
              </button>
            );
          })}
        </div>
    ...
    <div className="page order-details-page">
      ...
      <header className="order-detail-header">
        <div>
          ...
          <p>
            Received {order.received} · {order.age} ·{" "}
            <strong>
              {order.paid ? "Paid" : "Cash on delivery"}{" "}
        ...
        <div className="detail-actions">
          ...
          <button className="secondary-button" onClick={onKOT}>
            <Printer size={16} />
            Print KOT
          </button>
          {action && (
      ...
      <div className="order-detail-grid">
        <div className="detail-main-column">
          <section className="detail-section">
              ...
              {order.items.map((item) => (
                <div key={item.name} className="detail-item-row">
                  ...
                  <span>
                    ...
                    {item.instructions && (
                      <em>Item note: {item.instructions}</em>
                    )}
                  </span>
                </div>
              ))}
            ...
            {order.instructions && (
              <div className="prominent-instruction">
                <CircleAlert size={18} />
                <span>
                  <strong>Order instruction</strong>
                  {order.instructions}
            ...
            <div className="customer-detail-grid">
              <div>
                ...
                <div className="inline-actions">
                  <a href={`tel:${order.phone.replaceAll(" ", "")}`}>
                    <Phone size={15} />
                    Call
                  </a>
                  <button
            ...
            <dl>
              ...
              {order.rounding !== 0 && (
                <div>
                  ...
                  <dd>
                    {order.rounding > 0 ? "+" : "−"}
                    {formatMoney(Math.abs(order.rounding))}
                  </dd>
                </div>
              )}
          ...
          <section className="detail-section timeline-section">
            ...
            {order.cancellationReason && (
              <div className="cancelled-reason">
                ...
                <small>Manual refund responsibility recorded.</small>
              </div>
            )}
          </section>
        </aside>
          ...
          {order.items.map((item) => (
            <div key={item.name}>
              ...
              <span>
                ...
                {item.instructions && <em>NOTE: {item.instructions}</em>}
              </span>
            </div>
          ))}
        </div>
